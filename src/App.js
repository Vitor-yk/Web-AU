import logo from './logo.svg';
import './App.css';

function App() {
  return (
    /*  barra de navegação  */
    <div className="App">
      <nav class="navbar navbar-expand-lg navbar-dark bg-none">
        <div class="container-fluid">
          <a class="navbar-brand px-2" href="#">Logo</a>
          <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarText" aria-controls="navbarText" aria-expanded="false" aria-label="Toggle navigation">
            <span class="navbar-toggler-icon"></span>
          </button>
          <div class="collapse navbar-collapse" id="navbarText">
            <ul class="navbar-nav me-auto mb-2 mb-lg-0 ">
              <li class="nav-item" id="lista">
                <a class="nav-link " aria-current="page" href="#">Sobre</a>
              </li>
              <li class="nav-item">
                <a class="nav-link " href="#">Adotar</a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="#">Doações</a>
              </li>
            </ul>
            <span class="navbar-text">
              <a class="nav-link" href="#">Login</a>
            </span>
          </div>
        </div>
      </nav>

      <div id="services-area-animals">
        <div class="container">
          <div class="row">
            <div class="col-12">
              <h3 class="second-title text-center pt-5">Sessão catálogo de animais</h3>
            </div>

            <div class="col-md-12 text-center p-5" id="menuFiltros">
              <form>
                <div class="form">
                  <select class="custom-select custom-select-sm mb-3" id="Form1">
                    <option>1111111111</option>
                    <option>2222222222</option>
                    <option>3333333333</option>
                    <option>4444444444</option>
                    <option>5555555555</option>
                  </select>
                  <select class="custom-select custom-select-sm" id="Form2">
                    <option>1111111111</option>
                    <option>2222222222</option>
                    <option>3333333333</option>
                    <option>4444444444</option>
                    <option>5555555555</option>
                  </select>
                  <input type="search" class="custom-select custom-select-sm" id="search" placeholder="Pesquisa" required></input>
                </div>

              </form>
            </div>

            <div class="col-1">
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-1">
            </div>

            <div class="col-1">
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>


            <div class="col-1">
            </div>
            <div class="col-1">
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>


            <div class="col-1">
            </div>
            <div class="col-1">
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

            <div class="col-md-2 service-box">
              <a href="#">
                <img src="data:image/png;base64," width="180"></img>
                <div class="col-7 bg-dark" >
                  <h4 class="bg-none" id="nome">Nome</h4>
                </div>
              </a>
            </div>

          </div >
        </div >
      </div >
    </div >
  );
}

export default App;
